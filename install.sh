...
docker run --name master-db -d -p 5432:5432 --net test -e POSTGRES_DB=test -e POSTGRES_HOST_AUTH_METHOD=trust -e POSTGRES_USER=postgres -v /$PWD/postgres:/var/lib/postgresql/data postgres:latest
sleep 10
echo "master-db container running on port 5432"
...
docker exec -it master-db psql -U postgres -c "alter system set max_wal_size = '200MB';" && \
docker exec -it master-db psql -U postgres -c "alter system set wal_keep_segments = 8;" && \
...
docker run --name replica-db -d -p 5433:5432 -e POSTGRES_DB=test -e POSTGRES_HOST_AUTH_METHOD=trust -e POSTGRES_USER=postgres -v /$PWD/postgresslave:/var/lib/postgresql/data --net test postgres:latest

sleep 2
echo "replica-db container running on port 5433"
